...
void CreateBars(HWND hwnd,HINSTANCE hInstance)
{
  ...
  if (lstrlen(tchToolbarBitmap))
  {
    ...
    hbmp = (HBITMAP)LoadImage(NULL,szTmp,IMAGE_BITMAP,0,0,LR_CREATEDIBSECTION|LR_LOADFROMFILE);
  }
  if (hbmp)
  ...
  if (lstrlen(tchToolbarBitmapHot))
  {
    ...
    if (hbmp = (HBITMAP)LoadImage(NULL,szTmp,IMAGE_BITMAP,0,0,LR_CREATEDIBSECTION|LR_LOADFROMFILE))
    {
      GetObject(hbmp,sizeof(BITMAP),&bmp);
      himl = ImageList_Create(bmp.bmWidth/NUMTOOLBITMAPS,bmp.bmHeight,ILC_COLOR32|ILC_MASK,0,0);
      ImageList_AddMasked(himl,hbmp,CLR_DEFAULT);
        ...
        if (bCmdEnabled) {
          ...
          if (cchSelection > 0 && cchSelection <= 500 && SendMessage(hwndEdit,SCI_GETSELTEXT,0,0) < COUNTOF(mszSelection))
          {
            ...
            if (lstrlenA(mszSelection)) {
              ...
              lpszArgs = (LPWSTR)GlobalAlloc(GPTR,GlobalSize(lpszCommand));
...
void ParseCommandLine()
{
  ...
  while (bContinue && ExtractFirstArgument(lp3,lp1,lp2))
  {
    ...
    {
      LPWSTR lpFileBuf = (LPWSTR)LocalAlloc(LPTR,sizeof(WCHAR)*(lstrlen(lpCmdLine) + 1));
      ...
      lpFileArg = (LPWSTR)GlobalAlloc(GPTR,sizeof(WCHAR)*(MAX_PATH+2)); // changed for ActivatePrevInst() needs
...
BOOL RelaunchMultiInst() {
  ...
  if (flagMultiFileArg == 2 && cFileList > 1) {
    ...
    LPWSTR lp1 = (LPWSTR)LocalAlloc(LPTR,sizeof(WCHAR)*(lstrlen(lpCmdLineNew) + 1));
    LPWSTR lp2 = (LPWSTR)LocalAlloc(LPTR,sizeof(WCHAR)*(lstrlen(lpCmdLineNew) + 1));
