...
HRESULT PrivateSetCurrentProcessExplicitAppUserModelID(PCWSTR AppID)
{
  ...
  pfnSetCurrentProcessExplicitAppUserModelID = (SetCurrentProcessExplicitAppUserModelIDPROC)
...
void SetWindowTransparentMode(HWND hwnd,BOOL bTransparentMode)
{
  ...
  SetLayeredWindowAttributesPROC fp;
  int  iAlphaPercent;
  BYTE bAlpha;

  if (bTransparentMode) {
    if (fp = (SetLayeredWindowAttributesPROC)GetProcAddress(GetModuleHandle(L"User32"),"SetLayeredWindowAttributes")) {
...
{
  RECT rc;
  PRESIZEDLG pm = (PRESIZEDLG)GetProp(hwnd,L"ResizeDlg");
...
void MakeBitmapButton(HWND hwnd,int nCtlId,HINSTANCE hInstance,UINT uBmpId)
{
  ...
  BITMAP bmp;
  BUTTON_IMAGELIST bi;
  HBITMAP hBmp = (HBITMAP)LoadImage(hInstance,MAKEINTRESOURCE(uBmpId),IMAGE_BITMAP,0,0,LR_CREATEDIBSECTION);
...
{
  typedef BOOL (__stdcall *IsAppThemedPROC)();
  typedef HTHEME (__stdcall *OpenThemeDataPROC)(HWND, LPCWSTR);
  typedef HRESULT (__stdcall *GetThemeSysFontPROC)(HTHEME, int, LOGFONTW *plf);
  typedef HRESULT (__stdcall *CloseThemeDataPROC)(HTHEME hTheme);
  ...
  if (hModUxTheme = GetModuleHandle(L"uxtheme.dll")) {
    if (((IsAppThemedPROC)GetProcAddress(hModUxTheme,"IsAppThemed"))()) {
      hTheme = ((OpenThemeDataPROC)GetProcAddress(hModUxTheme,"OpenThemeData"))(NULL,L"WINDOWSTYLE;WINDOW");
      if (hTheme) {
        if (S_OK == ((GetThemeSysFontPROC)GetProcAddress(hModUxTheme,"GetThemeSysFont"))(hTheme,/*TMT_MSGBOXFONT*/805,&lf)) {
          if (lf.lfHeight < 0)
        ...
        ((CloseThemeDataPROC)GetProcAddress(hModUxTheme,"CloseThemeData"))(hTheme);
      }
    }
  }
